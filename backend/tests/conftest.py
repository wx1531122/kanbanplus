...
def registered_user(test_client, db_session):
    ...
    if response.status_code == 409 and ('Email already exists' in response.json.get('message', '') or 'Username already exists' in response.json.get('message', '')):
        from backend.app.models import User
        user = User.query.filter_by(email=user_details['email']).first()
        if not user: # Should not happen if 409 was due to this user
             pytest.fail(f"User registration conflict but user not found: {user_details['email']}")
    elif response.status_code != 201:
        pytest.fail(f"User registration failed with status {response.status_code}: {response.json}")
...
def auth_headers(test_client, registered_user):
    ...
    if login_response.status_code != 200:
        pytest.fail(f"Login failed for user {registered_user['email']}: {login_response.json}")
...
def created_project_data(test_client, auth_headers, db_session):
    ...
    response = test_client.post('/api/projects', headers={'Authorization': auth_headers['Authorization']}, json={
        'name': project_name,
...
def created_task_data(test_client, auth_headers, created_project_data, db_session):
    ...
    stage_response = test_client.post(f'/api/projects/{project_id}/stages', headers=headers, json={
        'name': stage_name
    })
    ...
    task_response = test_client.post(f'/api/stages/{stage_id}/tasks', headers=headers, json={
