import pytest
import json
from backend.app.models import User, Project, Stage, Task, Comment

# Fixtures `auth_headers` and `created_task_data` are now in conftest.py

# --- Test Cases for Comments API ---

def test_create_comment_success(test_client, auth_headers, created_task_data, db_session):
    task_id = created_task_data['task_id']
    request_headers = {'Authorization': auth_headers['Authorization']}
    comment_content = "This is a test comment."

    response = test_client.post(f'/api/tasks/{task_id}/comments', headers=request_headers, json={
...
def test_get_comments_for_task(test_client, auth_headers, created_task_data, db_session):
    task_id = created_task_data['task_id']
    request_headers = {'Authorization': auth_headers['Authorization']}
    
    # Create a couple of comments
    test_client.post(f'/api/tasks/{task_id}/comments', headers=request_headers, json={'content': 'First comment'})
    test_client.post(f'/api/tasks/{task_id}/comments', headers=request_headers, json={'content': 'Second comment'})

    response = test_client.get(f'/api/tasks/{task_id}/comments', headers=request_headers)
...
def test_create_comment_non_existent_task(test_client, auth_headers):
    ...
    response = test_client.post(f'/api/tasks/{non_existent_task_id}/comments', headers=request_headers, json={
        'content': 'A comment for a ghost task.'
    })
...
def test_create_comment_missing_content(test_client, auth_headers, created_task_data):
    ...
    response = test_client.post(f'/api/tasks/{task_id}/comments', headers=request_headers, json={'content': '   '}) # Whitespace only
    assert response.status_code == 400
    assert response.json['message'] == 'Comment content is required'
...
def test_comment_unauthorized_no_token(test_client, created_task_data):
    ...
    response_post = test_client.post(f'/api/tasks/{task_id}/comments', json={'content': 'Unauthorized comment'})
...
def another_user_auth_headers(test_client, db_session): # Added db_session
    ...
    assert login_response.status_code == 200, f"Login failed for another_user: {login_response.json}"
    access_token = login_response.json['access_token']
    return {'Authorization': f'Bearer {access_token}'}

def test_comment_on_others_task_forbidden(test_client, created_task_data, another_user_auth_headers):
    task_id = created_task_data['task_id'] # Task created by the first user (auth_headers fixture user)
    request_headers_another_user = another_user_auth_headers

    # Another user tries to comment
    response_post = test_client.post(f'/api/tasks/{task_id}/comments', headers=request_headers_another_user, json={
        'content': "Trying to comment on someone else's task"
    })
    assert response_post.status_code == 403
    assert response_post.json['message'] == 'Access forbidden to this task'

    # Another user tries to get comments
    response_get = test_client.get(f'/api/tasks/{task_id}/comments', headers=request_headers_another_user)
...
def test_get_comments_for_non_existent_task(test_client, auth_headers):
    ...
    response = test_client.get(f'/api/tasks/{non_existent_task_id}/comments', headers=request_headers)
    assert response.status_code == 404
    assert response.json['message'] == 'Task not found'
...
def test_create_comment_activity_log(test_client, auth_headers, created_task_data, db_session):
    task_id = created_task_data['task_id']
    project_id = created_task_data['project_id']
    task_content_ellipsis = created_task_data['task_content'][:30] + '...' # Match how it's logged
    request_headers = {'Authorization': auth_headers['Authorization']}
    ...
    activity_response = test_client.get(f'/api/projects/{project_id}/activities', headers=request_headers)
    ...
    for activity in activities:
        if activity['action_type'] == 'COMMENT_ADDED' and \
           activity['task_id'] == task_id and \
           activity['project_id'] == project_id and \
           activity['user_id'] == auth_headers['user_id']:
            assert f"commented on task '{task_content_ellipsis}'" in activity['description']
            comment_activity_found = True
            break
    assert comment_activity_found, "COMMENT_ADDED activity not found for new comment in project activities"

    # Check activity log for the task
    task_activity_response = test_client.get(f'/api/tasks/{task_id}/activities', headers=request_headers)
    ...
    for activity in task_activities:
        if activity['action_type'] == 'COMMENT_ADDED' and \
           activity['user_id'] == auth_headers['user_id']:
            ...
            break
    assert task_comment_activity_found, "COMMENT_ADDED activity not found for new comment on task activities"
