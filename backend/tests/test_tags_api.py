import pytest
import json
from backend.app.models import User, Project, Stage, Task, Tag, ActivityLog
...
def test_create_and_list_tags(test_client, auth_headers, db_session):
    ...
    # Create first tag
    response_create1 = test_client.post('/api/tags', headers=request_headers, json={'name': tag_name1})
    ...
    response_create_dup = test_client.post('/api/tags', headers=request_headers, json={'name': tag_name1.lower()})
    ...
    # Create second tag
    response_create2 = test_client.post('/api/tags', headers=request_headers, json={'name': tag_name2})
    ...
    assert len(tags) >= 2 # Could be more if other tests created tags and DB is not perfectly isolated by function
...
def test_create_tag_missing_name(test_client, auth_headers_tags):
    ...
    response_empty = test_client.post('/api/tags', headers=headers, json={'name': '   '})
    assert response_empty.status_code == 400
    assert response_empty.json['message'] == 'Tag name is required'

# POST /api/tasks/<task_id>/tags
def test_add_tag_to_task_by_id_and_name(test_client, auth_headers_tags, created_task_for_tags, db_session):
    headers = {'Authorization': auth_headers_tags['Authorization']}
    ...
    tag_create_res = test_client.post('/api/tags', headers=headers, json={'name': tag_name_existing})
    assert tag_create_res.status_code == 201
    existing_tag_id = tag_create_res.json['id']

    # 1. Add existing tag by ID
    response_add_by_id = test_client.post(f'/api/tasks/{task_id}/tags', headers=headers, json={'tag_id': existing_tag_id})
    ...
    project_activities = test_client.get(f'/api/projects/{project_id}/activities', headers=headers).json
    assert any(
        a['action_type'] == 'TAG_ADDED_TO_TASK' and 
        a['task_id'] == task_id and
        f"added tag '{tag_name_existing}'" in a['description']
    ...
    response_add_by_name = test_client.post(f'/api/tasks/{task_id}/tags', headers=headers, json={'tag_name': tag_name_new})
    ...
    project_activities_after_new_tag = test_client.get(f'/api/projects/{project_id}/activities', headers=headers).json
    assert any(
        a['action_type'] == 'TAG_ADDED_TO_TASK' and 
        a['task_id'] == task_id and
        f"added tag '{tag_name_new}'" in a['description']
    ...
    response_add_again = test_client.post(f'/api/tasks/{task_id}/tags', headers=headers, json={'tag_id': existing_tag_id})
    assert response_add_again.status_code == 200 # Should be idempotent or indicate already present
    assert len(response_add_again.json['tags']) == 2 # Count should not change

def test_add_tag_to_task_invalid_input(test_client, auth_headers_tags, created_task_for_tags):
    headers = {'Authorization': auth_headers_tags['Authorization']}
    task_id = created_task_for_tags['task_id']
    ...
    # Non-existent tag_id
    response_bad_id = test_client.post(f'/api/tasks/{task_id}/tags', headers=headers, json={'tag_id': 9999})
...
def test_add_tag_to_non_existent_task(test_client, auth_headers_tags):
    headers = {'Authorization': auth_headers_tags['Authorization']}
    response = test_client.post('/api/tasks/8888/tags', headers=headers, json={'tag_name': 'SomeTag'})
    assert response.status_code == 404
    assert response.json['message'] == 'Task not found'

def test_add_tag_to_task_unauthorized(test_client, created_task_for_tags): # No auth headers
    task_id = created_task_for_tags['task_id']
    response = test_client.post(f'/api/tasks/{task_id}/tags', json={'tag_name': 'UnauthorizedTag'})
    assert response.status_code == 401

# DELETE /api/tasks/<task_id>/tags/<tag_id>
def test_remove_tag_from_task(test_client, auth_headers_tags, created_task_for_tags, db_session):
    headers = {'Authorization': auth_headers_tags['Authorization']}
    task_id = created_task_for_tags['task_id']
    ...
    test_client.post(f'/api/tasks/{task_id}/tags', headers=headers, json={'tag_id': tag1_id})
    test_client.post(f'/api/tasks/{task_id}/tags', headers=headers, json={'tag_id': tag2_id})
    
    task_res_before_delete = test_client.get(f'/api/tasks/{task_id}', headers=headers)
    assert len(task_res_before_delete.json['tags']) == 2

    # Remove tag1
    response_delete = test_client.delete(f'/api/tasks/{task_id}/tags/{tag1_id}', headers=headers)
    ...
    project_activities = test_client.get(f'/api/projects/{project_id}/activities', headers=headers).json
    assert any(
        a['action_type'] == 'TAG_REMOVED_FROM_TASK' and 
        a['task_id'] == task_id and
        f"removed tag '{tag1_name}'" in a['description']
        for a in project_activities
    ), "TAG_REMOVED_FROM_TASK activity not found"

    # Attempt to remove a tag not on the task (or already removed)
    response_delete_again = test_client.delete(f'/api/tasks/{task_id}/tags/{tag1_id}', headers=headers)
    ...
    response_delete_non_tag = test_client.delete(f'/api/tasks/{task_id}/tags/7777', headers=headers)
    assert response_delete_non_tag.status_code == 404
    assert response_delete_non_tag.json['message'] == 'Tag with id 7777 not found'

def test_remove_tag_unauthorized_or_forbidden(test_client, auth_headers_tags, created_task_for_tags):
    headers = {'Authorization': auth_headers_tags['Authorization']}
    task_id = created_task_for_tags['task_id']
    tag_to_add_res = test_client.post('/api/tags', headers=headers, json={'name': "TempTagForDeleteTest"})
    tag_id = tag_to_add_res.json['id']
    test_client.post(f'/api/tasks/{task_id}/tags', headers=headers, json={'tag_id': tag_id})
    ...
    test_client.post('/api/auth/register', json={'username': 'other_tags', 'email': email_other, 'password': 'opass'})
    login_res_other = test_client.post('/api/auth/login', json={'email': email_other, 'password': 'opass'})
    headers_other_user = {'Authorization': f"Bearer {login_res_other.json['access_token']}"}
    
    response_forbidden = test_client.delete(f'/api/tasks/{task_id}/tags/{tag_id}', headers=headers_other_user)
    assert response_forbidden.status_code == 403


# Test Task.to_dict() includes tags
def test_task_to_dict_includes_tags(test_client, auth_headers_tags, created_task_for_tags, db_session):
    headers = {'Authorization': auth_headers_tags['Authorization']}
    task_id = created_task_for_tags['task_id']
...
def test_add_tag_by_mixed_case_name(test_client, auth_headers_tags, created_task_for_tags, db_session):
    headers = {'Authorization': auth_headers_tags['Authorization']}
    task_id = created_task_for_tags['task_id']

    # 1. Create a tag with a specific casing
    original_tag_name = "MixedCaseTag"
    tag_create_res = test_client.post('/api/tags', headers=headers, json={'name': original_tag_name})
    assert tag_create_res.status_code == 201
    ...
    response_add = test_client.post(f'/api/tasks/{task_id}/tags', headers=headers, json={'tag_name': mixed_case_name_to_add})
    ...
    assert task_data['tags'][0]['name'] == original_tag_name # Important: name should be original case

    # Verify no new tag was created
    tags_list_res = test_client.get('/api/tags', headers=headers)
    tags_in_db = tags_list_res.json
    assert sum(1 for t in tags_in_db if t['name'].lower() == original_tag_name.lower()) == 1

    # 3. Add another tag by name, this time a completely new one with mixed case
    new_mixed_name = "AnotherNewMix"
    response_add_new = test_client.post(f'/api/tasks/{task_id}/tags', headers=headers, json={'tag_name': new_mixed_name})
    assert response_add_new.status_code == 200
    task_data_new = response_add_new.json
    assert len(task_data_new['tags']) == 2
    
    added_tag_info = next(t for t in task_data_new['tags'] if t['name'] == new_mixed_name)
