...
def test_get_project_activities_success(
    ...
    comment_res = test_client.post(
        ...
        json={"content": "A test comment for activity"},
    )
...
def test_get_task_activities_success(
    test_client, auth_headers, created_task_data, db_session
):
    task_id = created_task_data["task_id"]
    ...
    comment_res = test_client.post(
        f"/api/tasks/{task_id}/comments",
        headers=request_headers,
        json={"content": "Another comment for task activity"},
    )
    assert comment_res.status_code == 201
    ...
    update_res = test_client.put(
        f"/api/tasks/{task_id}",
        headers=request_headers,
        json={"content": updated_task_content},
    )
...
def test_get_activities_non_existent_project(test_client, auth_headers):
    request_headers = {"Authorization": auth_headers["Authorization"]}
    response = test_client.get(
        "/api/projects/99999/activities", headers=request_headers
    )
...
def another_user_auth_headers_activity(
    test_client, db_session
):  # Renamed from general another_user_auth_headers
    ...
    assert register_response.status_code in [
        201,
        409,
    ]  # Allow for existing user in less isolated test runs
...
def test_get_activities_forbidden_other_user(
    test_client, created_task_data, another_user_auth_headers_activity
):
    project_id = created_task_data[
        "project_id"
    ]  # Created by first user (auth_headers fixture user)
    task_id = created_task_data["task_id"]  # Created by first user
    headers_other_user = (
        another_user_auth_headers_activity  # Token for a different user
    )

    response_project = test_client.get(
        f"/api/projects/{project_id}/activities", headers=headers_other_user
    )
    assert response_project.status_code == 403
    assert response_project.json["message"] == "Access forbidden to this project"

    response_task = test_client.get(
        f"/api/tasks/{task_id}/activities", headers=headers_other_user
    )
...
def test_record_activity_service_directly(db_session, auth_headers, created_task_data):
    ...
    log_entry = (
        ...
        .filter_by(action_type=action, description=desc)
        .first()
    )
...
def test_task_deletion_logs_activity(
    test_client, auth_headers, created_task_data, db_session
):
    project_id = created_task_data["project_id"]
    task_id = created_task_data["task_id"]
    ...
    delete_response = test_client.delete(
        f"/api/tasks/{task_id}", headers=request_headers
    )
    ...
    assert log_entry.user_id == auth_headers["user_id"]
