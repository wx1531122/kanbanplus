...
def test_successful_registration(test_client, db_session): # db_session for potential direct DB checks
...
def test_registration_missing_fields(test_client):
    response = register_user(test_client, 'testuser2', '', 'password123') # Missing email
    assert response.status_code == 400
    assert 'Missing username, email, or password' in response.json['message']

    response = register_user(test_client, '', 'test2@example.com', 'password123') # Missing username
    assert response.status_code == 400
    assert 'Missing username, email, or password' in response.json['message']

    response = register_user(test_client, 'testuser2b', 'test2b@example.com', None) # Missing password
    assert response.status_code == 400
    assert 'Missing username, email, or password' in response.json['message']


def test_registration_duplicate_username(test_client, db_session):
    register_user(test_client, 'testuser3', 'test3@example.com', 'password123') # First registration
    response = register_user(test_client, 'testuser3', 'test3_other@example.com', 'password123') # Duplicate username
    assert response.status_code == 409
    assert response.json['message'] == 'Username already exists'

def test_registration_duplicate_email(test_client, db_session):
    register_user(test_client, 'testuser4', 'test4@example.com', 'password123') # First registration
    response = register_user(test_client, 'testuser4_other', 'test4@example.com', 'password123') # Duplicate email
    assert response.status_code == 409
...
def test_protected_route_with_invalid_token(test_client):
    ...
    response = test_client.get('/api/protected', headers=headers)
    assert response.status_code == 422 # Expecting JWT error (invalid token format or content)
    assert 'msg' in response.json # Flask-JWT-Extended default error key
    assert response.json['msg'] in ['Invalid header padding', 'Not enough segments', 'Invalid token format'] or 'Invalid' in response.json['msg']
...
def test_logout(test_client, db_session):
    ...
    assert response.status_code == 200
    assert response.json['message'] == 'Logout successful. Please discard the token client-side.'
