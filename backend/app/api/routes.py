...
from backend.app.models import User, Project, Stage, Task, SubTask, Comment, ActivityLog, Tag # Import all models
...
def delete_task(task_id):
    ...
    record_activity(
        ...
        description=f"User '{user.username}' deleted task '{task.content[:30]}...' from stage '{task.stage.name}'",
        user_id=current_user_id,
        project_id=task.stage.project.id,
        task_id=task.id
    )
...
def get_project_activities(project_id):
    ...
    activities = ActivityLog.query.filter_by(project_id=project_id).order_by(ActivityLog.created_at.desc()).all()
...
def get_task_activities(task_id):
    ...
    if not task:
        return jsonify({'message': 'Task not found'}), 404
    if task.stage.project.user_id != current_user_id: # Check ownership via project
        return jsonify({'message': 'Access forbidden to this task'}), 403
...
def create_tag():
    ...
    try:
        db.session.commit()
    except IntegrityError: # Handles potential race conditions if another request creates the same tag
        db.session.rollback()
        existing_tag = Tag.query.filter(db.func.lower(Tag.name) == name.lower()).first()
        if existing_tag:
            return jsonify(existing_tag.to_dict()), 200
            ...
            return jsonify({'message': 'Error creating tag, possibly due to a conflict.'}), 500
...
def add_tag_to_task(task_id):
    ...
    if tag_id:
        ...
        tag_to_add = Tag.query.filter(db.func.lower(Tag.name) == tag_name.lower()).first()
        if not tag_to_add:
    ...
    if tag_to_add in task.tags:
        return jsonify({'message': 'Task already has this tag', 'task': task.to_dict(include_subtasks=True, include_tags=True)}), 200 # Or 409
...
def remove_tag_from_task(task_id, tag_id):
    ...
    if tag_to_remove not in task.tags:
        return jsonify({'message': 'Tag not found on this task'}), 404 # Or 204 if we consider it idempotent
